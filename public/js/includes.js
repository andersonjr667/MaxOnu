document.addEventListener("DOMContentLoaded", function () {
  ...
  const getRoleDestination = (user, delegationStatus) => {
    ...
    if (
      delegationStatus?.registrationOpen &&
      !delegationStatus?.registration?.submittedAt
    ) {
  ...
  const parseClassGroup = (classGroup = "") => {
    ...
    const grade =
      normalized.slice(separatorIndex + 3).trim() || "Não informada";
    return { unit, grade };
  };
  ...
  const fetchFreshAuthContext = async () => {
    ...
    try {
      ...
      if (!meResponse.ok) {
        console.error(
          "[AUTH] GET /api/me falhou com status:",
          meResponse.status,
        );
        clearStoredAuth();
        return null;
  ...
  const getAuthContext = async (options = {}) => {
    ...
    authContextPromise = fetchFreshAuthContext().finally(() => {
      authContextPromise = null;
    });
  ...
  const mergeNotifications = (context, storedNotifications) => {
    const pendingInvites =
      context?.delegationStatus?.notifications?.filter(
    ...
    return [...inviteCards, ...persisted].sort(
      ...
      ? `
                <div class="notification-list">
                    ${notifications
                      .map((item) => {
                        if (
                          item.type === "delegation-invite-pending" &&
                          item.payload
                        ) {
                          const inviterName =
                            item.payload.fromFullName ||
                            item.payload.fromUsername;
                          const inviterClass = parseClassGroup(
                            item.payload.fromClassGroup,
                          );
                          const inviterImageUrl =
                            item.payload.fromProfileImageUrl ||
                            (item.payload.fromGender === "feminino"
  ...
  const renderHeaderAuth = (navLinks, context) => {
    ...
    const mobilePrimary = navLinks?.querySelector(
      '[data-mobile-auth-slot="primary"]',
    );
    const mobileSecondary = navLinks?.querySelector(
      '[data-mobile-auth-slot="secondary"]',
    );
    ...
    if (adminLinks && context?.user) {
      const isAdminOrCoord =
        context.user.role === "admin" || context.user.role === "coordinator";
    ...
    if (!context) {
      ...
      mobilePrimary.innerHTML =
        '<a href="/login" class="nav-link mobile-auth-enter">Entrar</a>';
      mobileSecondary.innerHTML =
        '<a href="/login#register" class="nav-link mobile-auth-register">Cadastrar</a>';
    ...
    mobilePrimary.innerHTML = `
        ...
        `;
    mobileSecondary.innerHTML =
      '<button type="button" class="nav-link mobile-auth-enter mobile-auth-logout" data-logout-trigger>Sair</button>';
    mobilePrimary.hidden = false;
    mobileSecondary.hidden = false;
  ...
  const openRealtimeNotifications = () => {
    ...
    // Mobile: abrir a central full-screen (overlay) ao invés do painel do header.
    // Usamos o overlay dedicado do arquivo central-notificacoes-mobile.js
    ...
    source.addEventListener("notification", async (event) => {
      try {
        ...
        if (navLinks) {
          renderNotifications(
            headerContext || getStoredAuthContext(),
            notificationsCache,
          );
        }
      } catch (error) {
  ...
  const handleNotificationResponse = async (
    ...
    try {
      const response = await fetch(
        `/api/delegation/notifications/${notificationId}/respond`,
        {
          method: "POST",
          headers: {
  ...
  const initHeader = async () => {
    ...
    document.addEventListener("click", async function (event) {
      // Guard premium: evita múltiplos toggles em sequência rápida
      // Fechamentos via fora/ESC/links não devem competir com o clique que abre/aciona.
      const clickTarget = event.target;
      if (!clickTarget) return;
      if (
        clickTarget.closest(
          "[data-notification-toggle],[data-notification-action],[data-notification-read],[data-notification-read-all]",
        )
      ) {
        // permite seguir para os handlers específicos abaixo
      }
      ...
      if (toggle) {
        ...
        const isMobile = window.matchMedia("(max-width: 640px)").matches;
        if (isMobile && window.MaxOnuCentralNotificacoesMobile?.openOverlay) {
          // mobile: abrir overlay full-screen da central
          window.MaxOnuCentralNotificacoesMobile.openOverlay();
          return;
        }
      ...
      const readAllButton = event.target.closest(
  ...
  const loadInclude = async (src) => {
    ...
    console.error(
      `Failed to load include: ${src} (tried absolute and relative paths)`,
    );
    return false;
  };
